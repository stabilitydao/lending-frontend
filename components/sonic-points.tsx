...
import { playAudio, formatNumber } from "@/helpers";

import { seeds } from "@stabilitydao/stability";

const SonicPoints = (): JSX.Element => {
  ...
  const getUserData = async () => {
    try {
      const [sonicResponse, ringsResponse, siloResponse] = await Promise.all([
        ...
        axios.get(`https://points-api.rings.money/points/${address}`),
        axios.get(`${seeds[0]}/rewards/silo-points/${address}`),
      ]);
  ...
  return (
    <div className="relative">
      <button
        ref={buttonRef}
        className="bg-accent-900 h-10 sm:py-1 px-1 xl:px-3 rounded-xl sm:gap-3 flex items-center justify-center xl:justify-start w-[48px] xl:w-[160px]"
      ...
      {sonicBtn && (
        ...
        >
          ...
          <div className="flex items-center justify-between mt-3">
            <span className="text-[14px] text-white">Rank:</span>
            <span className="text-[#FDA73A]">{user.rank}</span>
          </div>
